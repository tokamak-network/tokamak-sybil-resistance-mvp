...
template BatchTx(nLevels) {
    ...
    signal input MIN_BALANCE;       // Minimum balance constraint
    signal input EXPLODE_AMOUNT;    // Explode constant amount
    ...
    //states.balance1 <== balance1;
    states.balance2 <== balance2;
    states.MIN_BALANCE <== MIN_BALANCE;
    ...
    balanceUpdater.effectiveExplodeAmount <== states.effectiveExplodeAmount;
    ...
    // D - Vouch processor 2 (to->from) - only used for explode
    ...
    processor4.newValue <== 0;  // Update to 0 only on Explode
