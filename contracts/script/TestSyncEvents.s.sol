...
contract TestSyncEvents is Script {
    ...
    function run() external {
        ...
        forgeBatchInternal(sybilContract);

        // Only proceed with deposit if forgeBatch succeeded
        if (sybilContract.lastForgedBatch() > lastForgedBatch) {
            // Test deposit
        ...
        lastForgedBatch = sybilContract.lastForgedBatch();
        forgeBatchInternal(sybilContract);
    ...
    function forgeBatchInternal(Sybil sybilContract) internal {
    ...
    function validCreateAccountDeposit() public pure returns (TxParams memory) {
        return TxParams({fromIdx: 0, loadAmountF: 100, amountF: 0, toIdx: 0});
    }
