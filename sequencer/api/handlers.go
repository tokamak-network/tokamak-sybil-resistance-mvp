...
func retSQLErr(err error, c *gin.Context) {
	...
	} else if sqlErr, ok := common.Unwrap(err).(*pq.Error); ok {
		retDupKey(sqlErr.Code)
	} else if sqlErr, ok := meddler.DriverErr(common.Unwrap(err)); ok {
		retDupKey(sqlErr.(*pq.Error).Code)
	} else if common.Unwrap(err) == sql.ErrNoRows {
...
func retBadReq(err error, c *gin.Context) {
	...
	if err, ok := err.(*apiError); ok {
		unwrapError := common.Unwrap(err.Err)
